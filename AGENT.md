...
internal/
  config/            env config (server: PORT, DATABASE_URL, FRONTEND_ORIGIN; enrich: LLM_BASE_URL, LLM_API_KEY, LLM_MODEL)
  database/          pgxpool connection pool
  db/                GENERATED sqlc code (do not edit) + queries/*.sql (hand-written)
  handler/           HTTP handlers (Handler struct + Register wires routes)
