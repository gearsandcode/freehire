# bayt-source Specification

## Purpose

Crawl Bayt.com — the dominant Gulf/MENA job board — into the catalogue. `bayt` is a
board-based multi-company aggregator: each configured entry is a country scope, and one
crawl walks that country's paginated listings, reading every posting's self-contained
schema.org `JobPosting` (so the employer comes from the posting). Bayt's Akamai/Cloudflare
edge rejects the default Go fingerprint, so the adapter crawls over the shared
Chrome-fingerprint transport.
