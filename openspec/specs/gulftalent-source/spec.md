# gulftalent-source Specification

## Purpose

Crawl GulfTalent.com — a major Gulf/MENA job board — into the catalogue. `gulftalent` is a
boardless multi-company aggregator: one crawl enumerates the site's sitemap index, follows
the job-posting shards, and reads each posting's self-contained schema.org `JobPosting` (so
the employer comes from the posting). GulfTalent's Akamai edge rejects the default Go
fingerprint, so the adapter crawls over the shared Chrome-fingerprint transport.
