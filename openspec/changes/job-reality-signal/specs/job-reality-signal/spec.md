...
#### Scenario: Existing jobs are reconciled through the derive + reindex path
- **WHEN** the reality dictionary or thresholds change
- **THEN** `cmd/backfill-derive` re-derives the signal on existing jobs and `make reindex` refreshes the facet, matching the other deterministic facets
