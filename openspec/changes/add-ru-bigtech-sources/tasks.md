...
- [ ] 3.1 Test: cursor pagination follows `next` (extract `cursor`, re-issue on the public
...
- [ ] 3.2 Implement `yandex.go` (`Provider()="yandex"`, board ∈ {ru,com}); register
      `NewYandex(c)` in `sources.All`; add `sources/yandex.yml` with `ru` and `com` entries

## 4. Ozon (`ozon`, boardless) (TDD)

- [ ] 4.1 Test: page pagination to `meta.totalPages` (`?page=N&limit=50`); keeps only
...
- [ ] 4.2 Implement `ozon.go` (boardless marker); register; add `sources/ozon.yml`

## 5. Wildberries (`rwb`, boardless) (TDD)

- [ ] 5.1 Test: offset pagination to `data.range.count` (`?limit=200&offset=N`); detail
...
- [ ] 5.2 Implement `rwb.go` (boardless); register; add `sources/rwb.yml`

## 6. Sber (`sber`, boardless) (TDD)

- [ ] 6.1 Test: `skip/take=200` pagination to `data.total`; body inline (no detail call) =
...
- [ ] 6.2 Implement `sber.go` (boardless), throttling between pages (relies on client
      429/5xx backoff for 503s); register; add `sources/sber.yml`

## 7. Alfa-Bank (`alfabank`, boardless) (TDD)

- [ ] 7.1 Test: `skip/take=100` pagination to `total`; body inline (`description` HTML);
...
- [ ] 7.2 Implement `alfabank.go` (boardless); register; add `sources/alfabank.yml`

## 8. Lamoda (`lamoda`, boardless) (TDD)

- [ ] 8.1 Test: offset pagination `pagination[limit]/[start]` to `meta.total`
...
- [ ] 8.2 Implement `lamoda.go` (boardless); register; add `sources/lamoda.yml`

## 9. Kuper (`kuper`, boardless) (TDD)

- [ ] 9.1 Test: page pagination using `pages` from the `category:"pagination"` block; body
      inline (`description`); `external_id=id`, location `city`, remote from `wf[]`,
      `posted_at=nil`; URL synthesized best-effort (confirm in §18)
- [ ] 9.2 Implement `kuper.go` (boardless); register; add `sources/kuper.yml`

## 10. Aviasales (`aviasales`, boardless) (TDD)

- [ ] 10.1 Test: single-call list (plain array); detail loader
...
- [ ] 10.2 Implement `aviasales.go` (boardless); register; add `sources/aviasales.yml`

## 11. Dodo (`dodo`, boardless) (TDD)

- [ ] 11.1 Test: single-call list, flatten `data[].items[]`; detail
...
- [ ] 11.2 Implement `dodo.go` (boardless); register; add `sources/dodo.yml` (URL best-effort, §18)

## 12. DomClick (`domclick`, boardless) (TDD)

- [ ] 12.1 Test: single-call list `https://career.domclick.ru/api/v1/vacancy/` (`result[]`);
...
- [ ] 12.2 Implement `domclick.go` (boardless); register; add `sources/domclick.yml`

## 13. MTS Link (`mtslink`, boardless) (TDD)

- [ ] 13.1 Test: single-call list `…/api/huntflow/vacancies` (keep `state=="OPEN"`); detail
...
- [ ] 13.2 Implement `mtslink.go` (boardless); register; add `sources/mtslink.yml`

## 14. T-Bank (`tbank`, boardless — POST) (TDD)

- [ ] 14.1 Test (body-aware fake): list `POST …/papi/getVacancies` body
...
- [ ] 14.2 Implement `tbank.go` (boardless, uses `PostJSON`); register; add `sources/tbank.yml`
      (URL best-effort, §18)

## 15. MTS (`mts`, boardless — POST + x-api-key) (TDD)

- [ ] 15.1 Test: list `POST …/v1/vacancies/filtered/career` body `{"offset":N,"limit":200}`
...
- [ ] 15.2 Implement `mts.go` (boardless): harvest the public key from the Nuxt runtime
      config at `Fetch` time; a harvest failure fails only this board. Register; add
      `sources/mts.yml`

## 16. Tochka (`tochka`, boardless — RSC detail) (TDD)
...
- [ ] 17.1 Test: list `…/career/api/v2/vacancies/?limit=50&offset=0` follows `next`/offset
...
- [ ] 17.2 Implement `vk.go` (boardless) with an isolated HTML-extraction helper; register;
      add `sources/vk.yml`

## 18. Registration sweep, URL confirmation, verification

- [ ] 18.1 Confirm all 15 `NewX(c)` lines are in `sources.All` and `reg`'s
      duplicate-provider guard passes; confirm all 15 `sources/<provider>.yml` files exist
...
- [ ] 18.3 `go build ./... && go vet ./... && go test ./internal/...` all green
