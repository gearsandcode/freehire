...
- [ ] 2.1 Test-first: `JobDocument` + `FromJob(db.Job)` mapping — decodes the
...
- [ ] 2.2 Implement `NewClient(url, key)` and `EnsureIndex(ctx)` that creates the
...
- [ ] 2.3 Implement `IndexJobs(ctx, []JobDocument)` (batched upsert by primary
  key) and `DeleteJob(ctx, id)`.
- [ ] 2.4 Implement `Search(ctx, SearchParams) (SearchResult, error)`: builds the
...
- [ ] 3.1 Add `cmd/reindex/main.go` mirroring `cmd/enrich`: `config.Load` →
...
- [ ] 3.2 If offset paging over `jobs` is insufficient, add a `ListJobsForIndex`
  query in `queries/jobs.sql` and `make sqlc`; otherwise reuse `ListJobs`. Verify
  `go build ./...` and `go vet ./...`.

## 4. Search endpoint

- [ ] 4.1 Test-first (handler test, httptest + fiber): `SearchJobs` parses `q`,
  ...
  not a panic.
- [ ] 4.2 Wire the search client into the `Handler` struct and `Register`; add
...
- [ ] 5.1 Add a thin search box to the Svelte SPA under `web/` that calls
...
- [ ] 6.2 Integration check against a real Meilisearch container (testcontainers
